...
class BandTrade(Strategy):
    ...
    async def handle_buy_side(
        ...
        for symbol, position in symbols_position.items():
            ...
            current_price = data_loader[symbol]
            from talib import MA_Type

            upper, middle, lower = talib.BBANDS(np.array(current_price.close).astype(float), matype=MA_Type.T3)

  
            # np_float_data = np.array(current_price.values)
            # print(type(np_float_data),"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")
            # # print(current_price,"CPPPPPCPCPCPCPCPCPCPCPCPCPCPCPCPCPCPCPCPCPCPCPCPCP")

            # # tlog(f"{symbol} -> {current_price}")
            # # # resampled_close = self.calc_close(symbol, data_loader, now)
            # # cp=current_pric
            # # print(cp,"HHHHHHHHHHIiiiiiiiiiiiiiiiHHHHHHHHHhiiiiiiiiiiiiiiiii")
            # bband = BBANDS(
            #     np_float_data
            # )
            # print("BBBBBBBBBBBBBBBBBAND",bband)
    ...
    async def handle_sell_side(
        ...
        actions['pnb'] = {
                    ...
                    "qty": str(position),
                    "type": "limit",
                    "limit_price": str(data_loader[symbol].close[-1]),
                }
    ...
    async def run(
        ...
        # if float(minute_history.close.iloc[-1]-5) > float(minute_history.close.iloc[-2]):
        if symbol.lower()=="ntpc":
            actions = {  "symbol":symbol,
                        "side": "BUY",
                        "qty": str(position),
                        "type": "limit",
                        "limit_price": minute_history.close.iloc[-1],
                    }
            print(actions,"KKKKKKKKKKKKKKLLLLLLLLLLLJJJJJJJJJJJJJJJJKKKKKKKKKKLLLLLLLLLLLLLL")
    ...
    def get_position(self, symbol: str) -> float:
        print(symbol,"OOOOOOOOOOOOOOOOPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP")
        return float(25.00)